...
- **GIGA R1 + Display** - winch operator UI. Talks to the ESP32 over the `GIGA UART` link (hardware UART0, see "Transports" below). Full command surface, full telemetry. Its own firmware (`firmware/giga_display/`) has started - WiFi, SD card, dashboard sync and on-device settings editing are built (see "GIGA Display Firmware" below) - but the actual operator UI over the ESP32 link above (tension display, state/e-stop controls) isn't built yet.
...
**Open design note, not yet implemented (handheld/Heltec firmware hasn't been started at all):** in `"winchman"` mode, the handheld is carried by the student being towed, but the **winchman**, not the student, holds tow authority (calibrate/release/etc. are GIGA/winchman-only regardless of mode - see "Boot Configuration" above). So the plan is for the handheld's own firmware to disable `deadman` and its other buttons (inert - a press does nothing) whenever it knows it's in `"winchman"` mode, since a student shouldn't need to understand or operate them. **Crucially, `lat`/`lon`/`baro_alt_m` reporting must keep going on schedule regardless of that button-disable** - those aren't coupled to `deadman`/button state, and if the handheld firmware naively silenced its whole `cmd` stream along with the buttons, the tow would lose its GPS/height log entirely (see "GPS + Logging" below). This also means whatever eventually consumes `effectiveDeadman()` in the real actuation loop (still unimplemented - see "Current Firmware Status") **must not** treat an inert winchman-mode handheld's permanent `deadman:false` as "pilot let go" the way it correctly does for a real solo-mode timeout - the deadman safety-gate as designed only makes sense in `"solo"` mode, where the handheld's button really is the sole safety authority.
...
- **Tow log** - a year-tabbed list of past tows plus a detail chart: height vs. **ground distance from the winch** (derived from `line_length_m` and height via Pythagoras, not time), release marked as the highest point, yellow dots at the state transitions (start, losing ground contact, normal tow starting, release), and a hover tooltip (state/height/distance/time-in-flight/`tension_kg`) with a crosshair, similar to Home Assistant's history graph. Currently showing placeholder tow records (`config/tows/*.json` in that repo) - no real tow has been logged yet since GIGA/handheld firmware doesn't exist.
- **GIGA sync ("To winch" / "From winch")** - confirmed working end-to-end against real GIGA hardware (see "GIGA Display Firmware" below): `GET /api/ping` and `GET`/`PATCH /api/config` are implemented on the GIGA and tested both directions (push an edited value, see it land on the GIGA's Tow Settings screens; edit on the GIGA, pull it back). `GET /api/tows` exists too but is still a `[]` stub - real tow-logging (position/height/tension written to the GIGA's SD card per tow) hasn't been built yet, so "From winch" always imports 0 tow records for now.
...
## GIGA Display Firmware

`firmware/giga_display/giga_display.ino` is the first real firmware for the GIGA node - same LVGL v9 stack as Arduino's own `LVGLDemo` example (`Arduino_H7_Video` + `Arduino_GigaDisplayTouch` + `lvgl`), landscape 800x480. What it covers so far is configuration and dashboard sync, not the actual winch/tow operator UI (see "Explicitly not yet implemented" below).

- **Homepage** - three mutually-exclusive states (idle / green nav menu / grey content screen), a gear icon opens the nav menu: **Network**, **Tow Profile**, **Calibration**, **PID**, **Cancel**. A small dot in the top-right corner shows live WiFi status (green/red), polled once a second independent of whatever screen is showing.
- **WiFi** - connects automatically at boot from `firmware/giga_display/secrets.h` (gitignored, `WIFI_SSID`/`WIFI_PASSWORD` need filling in locally, never committed), or manually via the Network menu (scan, pick from a list, on-screen-keyboard password entry, live `WiFi.begin()` status). Either path brings up the HTTP API below once connected.
- **SD card** - an external SPI reader wired to the GIGA's separate SPI1 header (`D89`/`D90`/`D91` + a `D10` CS pin, distinct from the board's main SPI pins - confirmed against a real GIGA on the Arduino forum, works with the plain `SD` library as-is). Holds one file, `/tow_config.json`, same field set as MQTT_Layout's `giga_boot_config.json`. Loaded at boot; rewritten whenever the config changes, from either the dashboard push or an on-device edit.
- **HTTP API** - implements the contract the MQTT Dashboard section above defines: `GET /api/ping`, `GET`/`PATCH /api/config`, `GET /api/tows` (`[]` stub for now - see above).
- **Tow Profile / Calibration / PID pages** - the boot-config tunables from "Boot Configuration" above, editable directly on the touchscreen and not just pushed from the dashboard, capped at 4 fields per page. Numeric fields use +/- stepper buttons rather than on-screen-keyboard typing; `operating_mode`/`use_saved_calibration` are single toggle buttons. Each page's Save button writes the whole config to SD and returns to the homepage in one tap - edits are already live over the HTTP API the moment they're made, Save only decides what survives a reboot.
...
- The Heltec firmwares (both winch-side and pilot-side) - not started.
- The GIGA's own operator UI (tension display, state-machine/e-stop controls, using the ESP32 `GIGA UART` link above) - not started; its WiFi/SD/dashboard-sync/settings firmware has (see "GIGA Display Firmware" above).
- Real per-tow logging on the GIGA (position/height/tension writes to the SD card, see "GPS + Logging" above) - `GET /api/tows` currently just returns an empty array.
