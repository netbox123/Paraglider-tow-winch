...
```json
{"type":"cmd","seq":42,"src":"giga","deadman":true,"tree_height":false,"state_cmd":"start_tow","tension_setpoint_kg":70,"fault_reset":false,"operating_mode":"winchman","pilot_weight_kg":85,"use_saved_calibration":false,"cal_raw_zero":123456,"cal_raw_100kg":234567,"under_tree_height_reduction_pct":40,"start_reduction_pct":20,"treeheight_to_full_tow_ramp_s":3,"start_to_treeheight_ramp_s":2,"release_before_taking_in_s":5,"pid_kp":1.2,"pid_ki":0.05,"pid_kd":0.01}
```
...
| `pilot_weight_kg` | number, optional | giga | set at boot, and again after every tow ends in `"winchman"` mode (see "Boot Configuration" below) - the current pilot's weight |
...
- `src:"giga"` can only affect `state_cmd`, `tension_setpoint_kg`, `fault_reset`, `operating_mode`, `pilot_weight_kg`, `use_saved_calibration`, `cal_raw_zero`, `cal_raw_100kg`, and the tow-config tunables above - `deadman`/`tree_height`/`lat`/`lon`/`baro_alt_m` from GIGA are likewise ignored, since those fields are meant to be the pilot's own live signal; letting the operator's UI set them too would defeat the point of the deadman check.
...
```json
{"type":"telemetry","seq":1337,"state":"NORMAL_TOW","tension_kg":68.4,"tension_setpoint_kg":70,"rope_out_m":210.5,"rpm":842,"motor_current_a":38.2,"fault":false,"fault_code":0,"line_cut":false,"cal_valid":true,"boot_configured":true,"operating_mode":"winchman","pilot_weight_kg":85,"tow_config_loaded":true,"lat":47.123456,"lon":8.123456,"baro_alt_m":42.3}
```
...
| `boot_configured` | bool | true once both `operating_mode` and `pilot_weight_kg` are set (see "Boot Configuration" below) - false blocks `calibrate`, so this tells the GIGA UI whether it still needs to prompt for them. In `"winchman"` mode this can go false again mid-session, after a release/fault clears `pilot_weight_kg` for the next pilot |
...
**Per-tow weight reset, `"winchman"` mode only:** after every `release` (successful tow end, from either the GIGA's `state_cmd` or the physical release switch - see [electronics.md](electronics.md)) or `emergency_stop`/local e-stop, the ESP32 forgets `pilot_weight_kg` again - `boot_configured` goes back to `false` and `calibrate` is refused until the GIGA sends it once more. This forces the winchman to actively (re-)confirm the weight for whoever tows next, rather than silently reusing the previous pilot's number. **In `"solo"` mode this reset is skipped** - it's the same pilot for the whole session, so the weight persists across repeat tows instead of forcing re-entry every time. `operating_mode` itself is never reset this way, only `pilot_weight_kg`.
...
- **`EMERGENCY_STOPPED`, `CALIBRATING` and the boot-configuration lock are real, guarded logic, not placeholders:**
  ...
  - `calibrate` is additionally refused until the GIGA has set both `operating_mode` and `pilot_weight_kg` this power-up (see "Boot Configuration" above) - a real, enforced lock, not just documented intent. `pilot_weight_kg` is deliberately forgotten again after every `release`/`emergency_stop` in `"winchman"` mode (kept in `"solo"` mode), so this lock genuinely re-engages between tows, not just once per power-up.
  ...
  - A local physical **release** switch (`GPIO2`, planned wiring via the winchman remote - see [electronics.md](electronics.md)) requests `state_cmd:"release"` on a press, through the exact same guarded `requestStateTransition()` path the GIGA's own `"release"` command uses - lets the winchman end a tow cleanly even with the GIGA link dead, matching the e-stop switch's independence from both UART links.
